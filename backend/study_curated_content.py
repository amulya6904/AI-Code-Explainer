...
def _section(heading: str, content: str, code_example: str):
    return {
        "heading": heading,
        "content": content.strip(),
        "code_example": cleandoc(code_example).strip() if code_example else None,
    }
            ...
            _section(
                ...
                public class Main {
                    ...
                    public static void main(String[] args) {
                        ...
                        item = null; // object becomes eligible for garbage collection
                        System.gc(); // request only, not a guarantee
...
CURATED_CHAPTER_IDS = frozenset(CURATED_FALLBACK_CONTENT)


def is_placeholder_study_content(chapter_id: str, content: dict) -> bool:
    if chapter_id not in CURATED_CHAPTER_IDS or not isinstance(content, dict):
        return False
    ...
    for section in sections:
        ...
        code = str(section.get("code_example") or "")
        combined = f"{text}\n{code}"
